...
class MyPyTutorialRegistry:
    ...
    def _load_tutorials(self) -> None:
        ...
        from storm_checker.tutorials.type_annotations_basics import (
            TypeAnnotationsBasics,
        )
    ...
    def list_tutorials(self) -> List[Dict[str, Any]]:
        ...
        for tutorial_id, tutorial_class in self._tutorials.items():
            ...
            progress_file = (
                get_data_directory() / "tutorial_progress" / f"{tutorial_id}.json"
            )
            is_completed = False
...
def list_mypy_tutorials(registry: MyPyTutorialRegistry) -> None:
    ...
    print(
        f"  {ColorPrinter.success(f'✅ Completed: {completed_count}/{total_count} tutorials ({progress_pct:.0f}%)')}"
    )
    if completed_count < total_count:
        remaining = total_count - completed_count
        print(
            f"  {ColorPrinter.info(f'📚 Remaining: {remaining} tutorials to master')}"
        )
    print()
    ...
    print(f"\n{BOLD}Usage:{RESET}")
    print(
        f"  {ColorPrinter.primary('stormcheck mypy tutorial <tutorial_id>')}  - Run a specific tutorial"
    )
    print(
        f"  {ColorPrinter.primary('stormcheck mypy tutorial --list')}         - Show this list"
    )
...
Examples:
  ...
  stormcheck mypy tutorial type_annotations_basics  # Master type annotations
    ...
    parser.add_argument("tutorial_id", nargs="?", help="ID of the MyPy tutorial to run")
    parser.add_argument(
        "-l",
        "--list",
        action="store_true",
