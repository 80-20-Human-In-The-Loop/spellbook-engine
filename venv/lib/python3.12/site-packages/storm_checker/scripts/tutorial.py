...
def list_tutorials(
    registry: GeneralTutorialRegistry, interactive: bool = True
) -> Optional[str]:
    """
    ...
    if interactive:
        try:
            ...
            for difficulty in sorted(by_difficulty.keys()):
                ...
                menu.add_header(
                    f"{diff_name} (Level {difficulty})",
                    color=diff_colors.get(difficulty, "#ccab78"),
                )
            ...
            print(
                f"  {ColorPrinter.learn('stormcheck mypy tutorial <name>')}     - Run MyPy tutorial"
            )
    ...
    if not interactive:
        ...
        # Display tutorials
        for difficulty in sorted(by_difficulty.keys()):
            diff_name = ["", "Beginner", "Easy", "Intermediate", "Advanced", "Expert"][
                difficulty
            ]
            diff_color = ["", "success", "success", "warning", "error", "error"][
                difficulty
            ]

            print(
                f"{getattr(ColorPrinter, diff_color)(f'{diff_name} (Level {difficulty}):', bold=True)}"
            )
            ...
            for tut in by_difficulty[difficulty]:
                ...
                print(
                    f"     {THEME['info']}⏱️  ~{tut['estimated_minutes']} minutes{RESET}{special}"
                )
                print()

        print(f"\n{BOLD}Usage:{RESET}")
        print(
            f"  {ColorPrinter.primary('stormcheck tutorial <tutorial_id>')}  - Run a specific tutorial"
        )
        print(
            f"  {ColorPrinter.primary('stormcheck tutorial --list')}        - Show this list"
        )

        # Add hint about MyPy tutorials
        print(f"\n{BOLD}For MyPy Type Safety Tutorials:{RESET}")
        print(
            f"  {ColorPrinter.learn('stormcheck mypy tutorial --list')}    - List MyPy typing tutorials"
        )
        print(
            f"  {ColorPrinter.learn('stormcheck mypy tutorial <name>')}     - Run MyPy tutorial"
        )
...
Examples:
  ...
  stormcheck tutorial pyproject_setup  # Learn about pyproject.toml configuration
    ...
    parser.add_argument("tutorial_id", nargs="?", help="ID of the tutorial to run")
    parser.add_argument(
        "-l",
        "--list",
        action="store_true",
    ...
    parser.add_argument(
        "--no-interactive", action="store_true", help="Disable interactive menu"
    )
