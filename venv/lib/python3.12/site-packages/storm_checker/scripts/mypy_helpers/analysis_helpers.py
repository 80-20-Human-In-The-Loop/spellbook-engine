...
def print_learning_path(analysis: AnalysisResult) -> None:
    ...
    for i, error in enumerate(analysis.learning_path[:5], 1):
        ...
        if explanation:
            print(
                f"   {ColorPrinter.success('💡 Quick fix:')} {explanation.simple_explanation}"
            )
...
def calculate_learning_progress(analysis: AnalysisResult) -> dict:
    ...
    easy_errors = len(analysis.by_difficulty.get(1, [])) + len(
        analysis.by_difficulty.get(2, [])
    )
    medium_errors = len(analysis.by_difficulty.get(3, []))
    hard_errors = len(analysis.by_difficulty.get(4, [])) + len(
        analysis.by_difficulty.get(5, [])
    )
