...
def print_storm_header(educational: bool = False) -> None:
    ...
    if educational:
        print_header(
            "Storm-Checker Type Safety Tool", "Learn Python typing through practice"
        )
    else:
...
def print_results_standard(
    ...
    ignore_note = (
        f" ({ignored_count} intentionally ignored)" if ignored_count > 0 else ""
    )
    print_warning(
    ...
    for category in sorted(analyzer.CATEGORIES, key=lambda c: c.difficulty):
        if category.id in analysis.by_category:
            ...
            print(
                f"{getattr(ColorPrinter, category_color)(category.name)} "
    ...
    if "uncategorized" in analysis.by_category:
        ...
        print(
            f"{ColorPrinter.error('Complex/Uncategorized Issues')} "
    ...
    if analysis.learning_path:
        ...
        complexity = (
            "Low" if difficulty <= 2 else "Medium" if difficulty <= 3 else "High"
        )
        ...
        print(
            f"{ColorPrinter.info(f'{error.file_path}:{error.line_number}')} - {error.message}"
        )
...
def print_results_educational(
    ...
    if config_errors:
        ...
        print(
            f"{ColorPrinter.info('Missing pyproject.toml or MyPy configuration issues detected.')}"
        )
        for error in config_errors[:2]:
            print(f"  • {error.message}")
        print()
...
def print_dashboard(
    ...
    if hasattr(tracker, "get_dashboard_data"):
        ...
        print(
            f"├─ Type Coverage: {overall_stats.get('type_coverage', {}).get('current', 0):.1f}%"
        )
        ...
        print(
            f"├─ Tutorials Completed: {tutorial_progress.get('completed', 0)}/{tutorial_progress.get('total', 10)}"
        )
        print(f"├─ Average Score: {tutorial_progress.get('average_score', 0):.0f}%")
        print(
            f"└─ Achievements Earned: {achievements.get('unlocked', 0)}/{achievements.get('total', 0)}"
        )
    ...
    if analysis.suggested_tutorials:
        ...
        print(
            f"1. Complete tutorial: stormcheck tutorial {analysis.suggested_tutorials[0]}"
        )
...
def print_next_steps_educational(
    ...
    if result.has_errors:
        ...
        easy_errors = [
            e
            for e in analysis.learning_path
            if any(
                cat.difficulty <= 2 and cat.matches_error(e)
