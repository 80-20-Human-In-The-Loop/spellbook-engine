...
class ProgressTracker:
    ...
    def _load_progress(self) -> ProgressData:
        ...
        now = datetime.now()
        return ProgressData(user_stats=UserStats(first_run=now, last_session=now))
    ...
    def _serialize_progress_data(self, progress: ProgressData) -> Dict[str, Any]:
        ...
        return {
            ...
            "tutorial_progress": {
                ...
                "last_activity": (
                    progress.tutorial_progress.last_activity.isoformat()
                    if progress.tutorial_progress.last_activity
                    else None
                ),
            },
            "achievements": {
                "unlocked": {
                    aid: dt.isoformat()
                    for aid, dt in progress.achievements.unlocked.items()
                },
    ...
    def _deserialize_progress_data(self, data: Dict[str, Any]) -> ProgressData:
        ...
        tutorial_progress = TutorialProgress(
            ...
            last_activity=(
                datetime.fromisoformat(tutorial_data["last_activity"])
                if tutorial_data.get("last_activity")
                else None
            ),
        )
    ...
    def end_session(self, time_spent: float) -> None:
        ...
        session_file = (
            self.sessions_dir
            / f"session_{self.current_session.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def record_tutorial_completion(
        self, tutorial_id: str, score: int, time_spent: float
    ) -> None:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        for aid, unlock_time in sorted(
            achievements.unlocked.items(), key=lambda x: x[1], reverse=True
        )[:3]:
        ...
        for achievement in self.achievements.values():
            ...
            if "errors_fixed" in achievement.requirement:
                ...
                if current >= target * 0.8:  # 80% there
                    ...
                    goals.append(
                        f"Fix {remaining} more errors for '{achievement.name}'"
                    )
                    break
    ...
    def clear_all_progress(self) -> Dict[str, int]:
        ...
        self.progress_data = ProgressData(
            user_stats=UserStats(first_run=now, last_session=now)
        )
