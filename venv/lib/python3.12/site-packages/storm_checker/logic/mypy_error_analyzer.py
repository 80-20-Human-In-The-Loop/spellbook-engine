...
class ErrorAnalyzer:
    ...
    def _group_by_difficulty(
        self, errors: List[MypyError], by_category: Dict[str, List[MypyError]]
        ...
        for category in sorted(self.CATEGORIES, key=lambda c: c.difficulty):
            if category.id in by_category:
                ...
                learning_path.extend(
                    sorted(with_explanation, key=lambda e: e.file_path)
                )
                learning_path.extend(
                    sorted(without_explanation, key=lambda e: e.file_path)
                )

        # Add any uncategorized errors at the end
        if "uncategorized" in by_category:
            learning_path.extend(
                sorted(by_category["uncategorized"], key=lambda e: e.file_path)
            )

        return learning_path

    def _calculate_complexity_score(
        self, by_difficulty: Dict[int, List[MypyError]], errors: List[MypyError]
    ) -> float:
    ...
    def generate_summary_report(self, analysis: AnalysisResult) -> str:
        ...
        for cat_id, errors in sorted_categories:
            ...
            if category:
                lines.append(
                    f"- **{category.name}**: {len(errors)} errors (Difficulty: {category.difficulty}/5)"
                )
            else:
        ...
        for i, error in enumerate(analysis.learning_path[:5], 1):
            lines.append(
                f"   - {error.file_path}:{error.line_number} - {error.message[:50]}..."
            )
        ...
        sorted_files = sorted(
            analysis.by_file.items(), key=lambda x: len(x[1]), reverse=True
        )[:3]
