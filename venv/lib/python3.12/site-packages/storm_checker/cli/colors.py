...
def create_rich_style(
    color_name: str, bold: bool = False, italic: bool = False, underline: bool = False
) -> Optional["RichStyle"]:
    ...
    return RichStyle(color=rich_color, bold=bold, italic=italic, underline=underline)
...
class EnhancedColorPrinter(ColorPrinter):
    ...
    def progress_text(current: int, total: int, label: str = "Progress") -> str:
        ...
        return EnhancedColorPrinter.rich_text(
            f"{label}: {current}/{total} ({percentage:.0f}%)", "primary"
        )
