...
class ProgressDashboard:
    ...
    def _render_header(self) -> str:
        ...
        lines.append(
            self.border.middle(
                self.width, center_text=f"{BOLD}STORM-CHECKER PROGRESS{RESET}"
            )
        )
    ...
    def _render_overall_stats(self, stats: Dict[str, Any]) -> str:
        ...
        lines.append(
            self.border.middle(
                self.width, left_text=f"{THEME['primary']}📊 Overall Statistics{RESET}"
            )
        )
        ...
        if coverage["improvement"] > 0:
            coverage_text += (
                f" ({THEME['success']}+{coverage['improvement']:.1f}%{RESET})"
            )
    ...
    def _render_week_activity(self, week_activity: List[Dict[str, Any]]) -> str:
        ...
        lines.append(
            self.border.middle(
                self.width, left_text=f"{THEME['info']}📈 This Week's Activity{RESET}"
            )
        )
    ...
    def _render_next_goals(self, goals: List[str]) -> str:
        ...
        lines.append(
            self.border.middle(
                self.width, left_text=f"{THEME['accent']}💡 Next Goals{RESET}"
            )
        )
        ...
        for i, goal in enumerate(goals):
            ...
            lines.append(self.border.middle(self.width, left_text=f"{prefix} {goal}"))
    ...
    def _render_footer(self, data: Dict[str, Any]) -> str:
        ...
        lines.append(
            self.border.middle(
                self.width, center_text=f"{THEME['text_muted']}{footer_text}{RESET}"
            )
        )
