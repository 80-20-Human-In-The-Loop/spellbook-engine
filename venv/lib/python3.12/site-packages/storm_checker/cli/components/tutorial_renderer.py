...
class TutorialRenderer:
    ...
    def __init__(self, width: Optional[int] = None):
        ...
        self.progress_bar = ProgressBar(
            width=30, style="blocks", color_filled="success", color_empty="text_muted"
        )
    ...
    def _get_terminal_width(self) -> int:
        ...
        try:
            ...
            return min(
                os.get_terminal_size().columns, 120
            )  # Increased for technical content
        except:
            return 80
    ...
    def render_question_screen(
        ...
        lines.append(
            self.border.middle(
                self.width, left_text=f"  {THEME['warning']}📝 Knowledge Check!{RESET}"
            )
        )
        lines.append(self.border.empty_line(self.width))
        ...
        if question_content:
            ...
            for line in content_lines:
                if line.strip():  # Non-empty lines
                    ...
                    max_content_width = (
                        self.width - 6
                    )  # 3 chars on each side for border + padding
                    ...
                    if len(self._strip_ansi(line)) > max_content_width:
                        ...
                        for wrapped_line in wrapped_lines:
                            lines.append(
                                self.border.middle(
                                    self.width, left_text=f"  {wrapped_line}"
                                )
                            )
                    else:
                        lines.append(
                            self.border.middle(self.width, left_text=f"  {line}")
                        )
                else:
    ...
    def render_result_screen(
        ...
        if result_data.get("explanation"):
            ...
            lines.append(
                self.border.middle(
                    self.width, left_text=f"  {THEME['info']}📖 Explanation:{RESET}"
                )
            )
            ...
            for word in words:
                ...
                if current_length + word_length + len(current_line) > max_width:
                    ...
                    if current_line:
                        ...
                        lines.append(
                            self.border.middle(self.width, left_text=f"  {line_text}")
                        )
                        current_line = [word]
    ...
    def _render_header(
        self, tutorial_id: str, title: str, page_info: str, is_completed: bool = False
    ) -> list:
    ...
    def _render_footer(
        self, current_page: int, total_pages: int, nav_hints: str
    ) -> list:
        ...
        progress_text = self.progress_bar.render(
            current_page, total_pages, label="Progress"
        )
    ...
    def _format_code_block(self, code_lines: List[str], language: str) -> List[str]:
        ...
        try:
            ...
            formatted_lines.extend(
                self._format_code_block_fallback(code_lines, language)
            )

        return formatted_lines

    def _format_code_block_fallback(
        self, code_lines: List[str], language: str
    ) -> List[str]:
        ...
        for line in code_lines:
            if language in ["python", "py"]:
                ...
                formatted_line = re.sub(
                    r'(["\'])([^"\']*)\1',
                    f"{THEME['success']}\\1\\2\\1{RESET}",
                    formatted_line,
                )
                # Color comments
                formatted_line = re.sub(
                    r"(#.*)", f"{THEME['text_muted']}\\1{RESET}", formatted_line
                )
                # Color keywords
                keywords = [
                    "def",
                ...
                for kw in keywords:
                    formatted_line = re.sub(
                        rf"\b({kw})\b", f"{THEME['accent']}\\1{RESET}", formatted_line
                    )
                ...
                formatted_line = re.sub(
                    r"(\[.*\])", f"{THEME['primary']}\\1{RESET}", formatted_line
                )
                # Color keys
                formatted_line = re.sub(
                    r"^(\s*)(\w+)(\s*=)",
                    f"\\1{THEME['learn']}\\2{RESET}\\3",
                    formatted_line,
                )
                # Color strings
                formatted_line = re.sub(
                ...
                # Color booleans
                formatted_line = re.sub(
                    r"\b(true|false)\b", f"{THEME['accent']}\\1{RESET}", formatted_line
                )
    ...
    def _get_navigation_hints(self, has_question: bool) -> str:
        ...
        if has_question:
            return (
                f"{THEME['warning']}[Enter: Knowledge Check | b: Back | q: Quit]{RESET}"
            )
        else:
