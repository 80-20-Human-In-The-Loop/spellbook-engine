...
class KeyboardHandler:
                ...
                for _ in range(10):  # Max sequence length
                    if select.select([sys.stdin], [], [], 0.1)[0]:
                        ...
                        if sequence in self.key_sequences:
                            return KeyPress(
                                key=self.key_sequences[sequence], raw_sequence=sequence
                            )
                    else:
                        break
    ...
    def _parse_key(self, char: str) -> KeyPress:
        ...
        if char == " ":
            ...
            return KeyPress(key=KeyCode.UNKNOWN, char=char, shift=char.isupper())
        else:
    ...
    def _parse_control_char(self, char: str) -> KeyPress:
        ...
        if ord_char == 10 or ord_char == 13:  # \n or \r
            ...
            return KeyPress(key=KeyCode.UNKNOWN, char=ctrl_char, ctrl=True)
        else:
            return KeyPress(key=KeyCode.UNKNOWN, raw_sequence=char)
    ...
    def create_input_loop(
        self, prompt: str = "", quit_keys: Optional[List[str]] = None
    ) -> Callable:
        """
        ...
        class InputLoop:
            ...
            def run(self) -> Optional[KeyPress]:
                ...
                if key_press:
                    ...
                    if pattern in quit_keys or (
                        key_press.char and key_press.char.lower() in quit_keys
                    ):
                        self.running = False
                        return None
...
def wait_for_specific_key(keys: List[str], prompt: str = "Press a key: ") -> KeyPress:
...
def demo_keyboard_handler():
    ...
    handler.bind_key(
        "h",
        lambda k: print("Help: Arrow keys navigate, F1-F12 for functions, 'q' to quit"),
    )
