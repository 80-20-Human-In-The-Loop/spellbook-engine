...
class ProgressBar:
    ...
    def render(
        ...
        if partial_block > 0 and filled_blocks < self.width:
            ...
            partial_char = self.style["partial"][
                min(partial_index, len(self.style["partial"]) - 1)
            ]
            bar_parts.append(f"{self.color_filled}{partial_char}{RESET}")
            filled_blocks += 1
        ...
        for value, color_name in segments:
            ...
            if segment_width > 0:
                ...
                if position <= current < position + value:
                    ...
                    if empty > 0:
                        parts.append(
                            f"{self.color_empty}{self.style['empty'] * empty}{RESET}"
                        )
                elif current >= position + value:
                    # Fully filled segment
                    parts.append(
                        f"{color}{self.style['filled'] * segment_width}{RESET}"
                    )
                else:
                    # Empty segment
                    parts.append(
                        f"{self.color_empty}{self.style['empty'] * segment_width}{RESET}"
                    )
...
def demo():
    ...
    for i in range(0, 101, 10):
        print(
            f"\r{segmented.render_segmented(segments, i, 'Tutorial Progress')}",
            end="",
            flush=True,
        )
        time.sleep(0.2)
