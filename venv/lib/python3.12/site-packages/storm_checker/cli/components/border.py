...
class Border:
    ...
    def top(self, width: int) -> str:
        ...
        if self.show_left and self.show_right:
            ...
            return self._colored(self.chars["tl"] + self.chars["h"] * (width - 1))
        elif self.show_right:
            return self._colored(self.chars["h"] * (width - 1) + self.chars["tr"])
        else:
    ...
    def bottom(self, width: int) -> str:
        ...
        if self.show_left and self.show_right:
            ...
            return self._colored(self.chars["bl"] + self.chars["h"] * (width - 1))
        elif self.show_right:
            return self._colored(self.chars["h"] * (width - 1) + self.chars["br"])
        else:
            return self._colored(self.chars["h"] * width)
    ...
    def middle(
        ...
        if center_text:
            ...
            line = (
                ...
                + (" " * 1 if left_text and self.show_left else "")
                + left_text
                ...
                * (right_pad - right_len - (1 if right_text and self.show_right else 0))
                + right_text
                + (" " * 1 if right_text and self.show_right else "")
            ...
            middle_space = (
                ...
                - left_len
                - right_len
                - (left_padding if left_text else 0)
                - (right_padding if right_text else 0)
            )
            line = (
                (self._colored(self.chars["v"]) if self.show_left else "")
                + (" " * 1 if left_text and self.show_left else "")
    ...
    def horizontal_divider(self, width: int) -> str:
        ...
        if self.show_left and self.show_right:
            return self._colored(
                self.chars["t_right"]
                + self.chars["h"] * (width - 2)
                + self.chars["t_left"]
            )
        elif self.show_left:
            return self._colored(self.chars["t_right"] + self.chars["h"] * (width - 1))
        elif self.show_right:
            return self._colored(self.chars["h"] * (width - 1) + self.chars["t_left"])
        else:
            return self._colored(self.chars["h"] * width)
    ...
    def box(
        ...
        if width is None:
            ...
            ansi_escape = re.compile(r"\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])")
            max_len = (
                max(len(ansi_escape.sub("", line)) for line in content)
                if content
                else 0
            )
        ...
        for line in content:
            ...
            if align == "center":
                ...
                formatted = (
                    " " * (padding + left_space) + line + " " * (padding + right_space)
                )
            elif align == "right":
                formatted = (
                    " " * (available - line_len + padding) + line + " " * padding
                )
            else:  # left
                formatted = (
                    " " * padding + line + " " * (available - line_len + padding)
                )
...
def demo():
    ...
    print(border.top(80))
    print(
        border.middle(80, "TUTORIAL: hello_world", "Introduction to Type Hints", "1/4")
    )
