...
class InteractiveMenu:
    ...
    def add_header(self, text: str, color: Optional[str] = None) -> None:
        ...
        self.items.append(MenuItem(text=text, type=MenuItemType.HEADER, color=color))

    def add_separator(self) -> None:
        """Add a separator line."""
        self.items.append(MenuItem(text="", type=MenuItemType.SEPARATOR))
    ...
    def _get_selectable_indices(self) -> List[int]:
        ...
        return [
            i for i, item in enumerate(self.items) if item.type == MenuItemType.NORMAL
        ]
    ...
    def _render_menu(self) -> str:
        ...
        for i, item in enumerate(self.items):
            ...
            if is_selected:
                ...
                lines.append(
                    f"[{fg_color} on {bg_color}] ▶ {item_text} [/{fg_color} on {bg_color}]"
                )
        ...
        if self.show_instructions:
            lines.append("")
            lines.append(
                "[dim]Navigate with ↑↓ arrows • Select with Enter • Exit with q/Esc[/dim]"
            )
